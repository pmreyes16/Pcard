...
export default function CardPreview({ card }: CardPreviewProps) {
  ...
  return (
    <div className="rounded-xl sm:rounded-2xl shadow-2xl p-4 sm:p-6 lg:p-8 text-white max-w-md mx-auto" style={gradientStyle}>
      <div className="flex flex-col items-center space-y-4 sm:space-y-6">
        {/* Profile Picture */}
        <div className="relative">
          <img
            src={card.profile_picture_url || 'https://d64gsuwffb70l.cloudfront.net/68dfdf1e98fe75cad01b712c_1759502163667_544acd21.webp'}
            alt={card.full_name || 'Profile'}
            className="w-32 h-32 sm:w-40 sm:h-40 lg:w-56 lg:h-56 rounded-none object-cover border-4 border-white shadow-lg"
          />
        </div>
        
        {/* Name and Title */}
        <div className="text-center space-y-1 sm:space-y-2">
          <h2 className="text-xl sm:text-2xl lg:text-3xl font-bold">{card.full_name || 'Your Name'}</h2>
          {card.job_title && (
            <p className="text-white/90 text-base sm:text-lg font-medium">{card.job_title}</p>
          )}
          {card.company && (
            <p className="text-white/80 text-sm sm:text-lg">{card.company}</p>
          )}
        </div>
        ...
        {card.bio && (
          <div className="text-center">
            <p className="text-white/90 text-sm leading-relaxed">{card.bio}</p>
          </div>
        )}
        ...
        <div className="w-full space-y-3 text-sm">
          ...
          {card.address && (
            <div className="flex items-center space-x-3">
              <span className="text-white/90">📍</span>
              <span className="text-white/90 text-xs leading-relaxed">{card.address}</span>
            </div>
          )}
        </div>
        ...
        <div className="text-center pt-4">
          <span className="text-white/60 text-xs">Live Preview</span>
        </div>
      </div>
    </div>
  );
